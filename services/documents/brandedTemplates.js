...
// Uses organization theme (primary/secondary/accent) and logo
...
function agreementTemplate({ org, vars }) {
  ...
  <div class="hero">
    ...
    <p>Between ${vars.employeeName || "Employee"} and ${org.companyName || "Company"} — ${vars.date || new Date().toLocaleDateString()}</p>
  ...
  const body = `
    <div class="section">
      ...
      <div class="card list">
        <ol>
          <li>Position: ${vars.position || "Role"}; Start Date: ${vars.startDate || vars.date || "TBD"}.</li>
          <li>Compensation: ${vars.salary || "As Discussed"}; Benefits as per company policy.</li>
    ...
    <div class="section row">
      <div class="col card">
        <strong>Company</strong><br/>${org.companyName || "Company"}<br/>Authorized Signatory
      </div>
      <div class="col card">
        <strong>Employee</strong><br/>${vars.employeeName || "Employee"}<br/>Signature
      </div>
    </div>
    <div class="footer">This document is auto‑generated by ${org.companyName || "our"} OMS.</div>
  `;
...
function simpleLetter({ org, vars, title }) {
  ...
  <div class="hero">
    <h1>${title}</h1>
    <p>${vars.date || new Date().toLocaleDateString()}</p>
    ${abstractShapes(org.theme?.primaryColor || "#000", org.theme?.accentColor || "#3B82F6")}
  </div>`;
  const body = `
    <div class="section">
      <h2>Recipient</h2>
      <div class="card">${vars.recipientName || vars.employeeName || "Employee"}</div>
    </div>
    <div class="section">
      <h2>Message</h2>
      <div class="card" style="line-height:1.7">${vars.body || "This letter confirms the subject as per records."}</div>
    </div>
    <div class="section row">
      <div class="col card">
        <strong>${org.companyName || "Company"}</strong><br/>HR Department
      </div>
      <div class="col card">
...
export function getTemplateHTML(type, org, vars = {}) {
  switch (type) {
    case "tpl_agreement_modern":
      return agreementTemplate({ org, vars });
    case "tpl_increment_letter":
      return simpleLetter({ org, vars, title: "Increment Letter" });
    case "tpl_recommendation":
      return simpleLetter({ org, vars, title: "Recommendation Letter" });
    case "tpl_experience":
      return simpleLetter({ org, vars, title: "Experience Letter" });
    default:
      return simpleLetter({ org, vars, title: "Company Letter" });
  }
}
